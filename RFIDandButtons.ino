...
char dataString[50] = {0}; //Data that sends through serial
...
int nextButtonPin = 7; //Next button bounded to pin 7
int prevButtonPin = 2; //Previous button bounded to pin 2
int useButtonPin = 4; //Use button bounded to pin 4
...
void loop()
{
  ...
  if(digitalRead(nextButtonPin)){
    sprintf(dataString,"%02X", "N1");
    Serial.println(dataString);
    delay(1000);
  }

  if(digitalRead(prevButtonPin)){
    sprintf(dataString,"%02X", "P1");
    Serial.println(dataString);
    delay(1000);
  }

  if(digitalRead(useButtonPin)){
    sprintf(dataString,"%02X", "U1");
    Serial.println(dataString);
    delay(1000);
  }
	...
	if (status == MI_OK)
	{
          ...
          //  Serial.println(checksum1);
            ...
            if(str[0] == 224)                      //You can change this to the first byte of your tag by finding the card's ID through the Serial Monitor
            {
                sprintf(dataString,"%02X", "T1");
                Serial.println(dataString);
